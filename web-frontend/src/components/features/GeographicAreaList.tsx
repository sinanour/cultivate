...
import TreeView, {
  type TreeViewProps,
} from "@cloudscape-design/components/tree-view";
...
export function GeographicAreaList() {
  ...
  const { data: filteredArea } = useQuery({
    ...
    queryFn: () =>
      GeographicAreaService.getGeographicAreaById(selectedGeographicAreaId!),
  ...
  const fetchChildrenBatch = useCallback(
    async (parentId: string, page: number = 1) => {
      try {
        const response = await GeographicAreaService.getChildrenPaginated(
          parentId,
          page,
          BATCH_SIZE,
          selectedGeographicAreaId,
  ...
  const loadChildrenBatched = useCallback(
  ...
  const loadNextBatch = useCallback(
    async (parentId: string) => {
      ...
      try {
        ...
        const allLoadedChildren = [
          ...currentState.loadedChildren,
          ...response.data,
        ];
      ...
      areas.forEach((area) => {
        ...
        if (!area.parentGeographicAreaId) {
          ...
          if (parentNode) {
            ...
            const cachedChildren = childrenCache.get(
              area.parentGeographicAreaId,
            );
            const batchedChildren =
              batchLoadingState[area.parentGeographicAreaId]?.loadedChildren;

            if (cachedChildren) {
              // Parent has cached children - rebuild from cache
              parentNode.children = cachedChildren.map((child) =>
                buildNodeWithCache(child),
              );
            } else if (batchedChildren) {
              // Parent has batched children - rebuild from batched state
              parentNode.children = batchedChildren.map((child) =>
                buildNodeWithCache(child),
              );
            ...
            if (
              !cachedChildren &&
              !batchedChildren &&
              !parentNode.children.find((c) => c.id === node.id)
            ) {
  ...
  const handleToggleItem = async (event: {
      ...
      content: (
        ...
        >
          <SpaceBetween direction="horizontal" size="xs">
            ...
            <Badge color={getAreaTypeBadgeColor(area.areaType)}>
              {area.areaType}
            </Badge>
            {isAncestor && <Badge color="grey">Read-Only</Badge>}
            ...
            {loadingState &&
              !loadingState.isCancelled &&
              loadingState.isLoading &&
              !node.loadingProgress && <Badge color="blue">Loading...</Badge>}
          </SpaceBetween>
        </div>
      ),
      actions:
