...
import { GeographicAuthorizationForm } from '../components/features/GeographicAuthorizationForm';
  ...
  return (
    <ContentLayout
      header={
        <Header
          variant="h1"
          description={user ? `Manage geographic authorization rules for ${user.email} (${user.role})` : 'Manage geographic authorization rules'}
          actions={
      ...
      breadcrumbs={
        <BreadcrumbGroup
          items={[
            ...
            { text: 'Authorizations', href: '#' },
          ]}
          onFollow={(e) => {
            e.preventDefault();
            if (e.detail.href === '/dashboard') navigate('/dashboard');
            if (e.detail.href === '/users') navigate('/users');
          }}
        />
      }
    >
      <SpaceBetween size="l">
        ...
        {(rulesError || areasError) && (
          <Alert type="error" header="Error Loading Authorization Data">
            {rulesError instanceof Error ? rulesError.message : areasError instanceof Error ? areasError.message : 'Failed to load authorization data'}
          </Alert>
        )}
        ...
        <Container
          header={
            <Header
              actions={
                <Button variant="primary" onClick={() => setShowAddForm(true)} disabled={rulesLoading}>
                  Add Rule
                </Button>
              }
            >
